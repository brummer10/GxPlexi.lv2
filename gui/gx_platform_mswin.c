...
void gx_gui_create_window_and_surface(gx_plexiUI *ui) {
	...
	static TCHAR szClassName[] = TEXT("gx_plexiUIClass");
	WNDCLASS wndclass = {0};
...
void gx_gui_destroy_main_window(gx_plexiUI *ui) {
	...
	UnregisterClass(TEXT("gx_plexiUIClass"), NULL);
}
...
void event_handler(gx_plexiUI *ui) {
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	gx_plexiUI *ui = (gx_plexiUI *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
