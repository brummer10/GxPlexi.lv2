...
static void resize_event(gx_plexiUI *ui);
...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_plexiUI* ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const struct _LV2UI_Descriptor * descriptor,
	...
	ui->controls[1] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {110, 30, 61, 61}, false,"BASS", KNOB, BASS};
	ui->controls[2] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {180, 30, 61, 61}, false,"MASTER", KNOB, MASTER};
	ui->controls[3] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {250, 30, 61, 61}, false,"MID", KNOB, MID};
	ui->controls[4] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {320, 30, 61, 61}, false,"PRESENSE", KNOB, PRESENSE};
	ui->controls[5] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {390, 30, 61, 61}, false,"TREBLE", KNOB, TREBLE};
	ui->controls[6] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {460, 30, 61, 61}, false,"VOLUME", KNOB, VOLUME};
...
static void cleanup(LV2UI_Handle handle) {
	gx_plexiUI* ui = (gx_plexiUI*)handle;
	cairo_destroy(ui->cr);
...
static void knob_expose(gx_plexiUI *ui,gx_controller* knob) {
...
static void bypass_expose(gx_plexiUI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(gx_plexiUI *ui, gx_controller* controller) {
...
static void _expose(gx_plexiUI *ui) {
	const char* plug_name = "GxPlexi" ;
...
static void controller_expose(gx_plexiUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
...
static void send_controller_event(gx_plexiUI *ui, int controller) {
...
static void check_value_changed(gx_plexiUI *ui, int i, float* value) {
...
static void check_is_active(gx_plexiUI *ui, int i, bool set) {
...
static bool aligned(int x, int y, gx_controller *control, gx_plexiUI *ui) {
...
static bool get_active_ctl_num(gx_plexiUI *ui, int *num) {
	bool ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static bool get_active_controller_num(gx_plexiUI *ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
static void scroll_event(gx_plexiUI *ui, int direction) {
	float value;
	int num;
...
static void enum_event(gx_plexiUI *ui, int i) {
	float value;
...
static void motion_event(gx_plexiUI *ui, double start_value, int m_y) {
...
static void set_key_value(gx_plexiUI *ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void key_event(gx_plexiUI *ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
static void set_previous_controller_active(gx_plexiUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void set_next_controller_active(gx_plexiUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
static void get_last_active_controller(gx_plexiUI *ui, bool set) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void event_handler(gx_plexiUI *ui) {
	XEvent xev;
